...
def train():
    ...
    for episode_batch_start in range(0, NUM_EPISODES, UPDATE_EVERY_EPISODES):
        ...
        for _ in range(UPDATE_EVERY_EPISODES):
            ...
            ep_rewards, ep_log_probs, ep_values = [], [], []
            done = False
            ...
            while not done:
                ...
                ep_values.append(value.squeeze(-1).squeeze(0))           
        ...
        advantage = (R - values).detach()

        actor_loss  = -(logp * advantage).mean()
        critic_loss = nn.functional.mse_loss(values, R)
    ...
    torch.save(model.state_dict(), "part2_pong_ac_policy.pth")
    env.close()
...
def evaluate_and_plot(episode_rewards, trained_model):
    ...
    plt.title('Part 2: Pong w/ Baseline — Training Rewards')
    ...
    plt.title(f'Part 2: Pong w/ Baseline — Evaluation ({EVAL_EPISODES} Episodes)')
    plt.xlabel('Total Reward'); plt.ylabel('Frequency'); plt.legend(); plt.grid(True)

    plt.tight_layout()
    plt.savefig('part2_pong_baseline_results.png')
    plt.show()
